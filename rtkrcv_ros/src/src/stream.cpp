...
#include <ros/ros.h>
#include <string>
#include "rtkrcv_ros/states_tow.h"
#include "rtkrcv_ros/states_hms.h"
#include "rtkrcv_ros/nmea.h"
...
static ros_pub_t *openRosPub(const char *path, int out_format, int time_format, char *msg)
{
    ...
    if(out_format==0 || out_format==1 || out_format==2 || out_format==5){
        ros_pub->time_format=time_format;
        if(time_format==1){
            ros_pub->publisher=node->advertise<rtkrcv_ros::states_hms>(path,10);
            return ros_pub;
        }else{
            ros_pub->publisher=node->advertise<rtkrcv_ros::states_tow>(path,10);
            return ros_pub;
        }
    }else if(out_format==3){
        ros_pub->publisher=node->advertise<rtkrcv_ros::nmea>(path,10);
        return ros_pub;
    }
}
    ...
    if(n>0){
        ...
        if(ros_pub->output_format==0 || ros_pub->output_format==1 || ros_pub->output_format==2){
            ...
            if(ros_pub->time_format==1){
                rtkrcv_ros::states_hms msg;
                msg.header.stamp=ros::Time::now();
                sscanf ((char*)buff,"%lu/%lu/%lu\t%lu:%lu:%lf\t%lf\t%lf\t%lf\t%d\t%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf",&msg.year, &msg.month, &msg.day, &msg.hour,
                            &msg.min, &msg.sec, &msg.data[0], &msg.data[1], &msg.data[2], &msg.Q, &msg.ns, &msg.std[0],	&msg.std[1],
                            &msg.std[2],&msg.std[3],&msg.std[4],&msg.std[5], &msg.age, &msg.ratio);

                if(ros_pub->output_format==0){
                    msg.header.frame_id=std::string("LLA");
                    double aux=msg.std[1];
                    msg.std[0]=aux;
                    aux=msg.std[4];
                    msg.std[4]=msg.std[5];
                    msg.std[5]=aux;
                }else if(ros_pub->output_format==1){
                    msg.header.frame_id=std::string("ECEF");
                }else if(ros_pub->output_format==2){
                    msg.header.frame_id=std::string("ENU");
                }

                ros_pub->publisher.publish(msg);
            }else{
                rtkrcv_ros::states_tow msg;
                msg.header.stamp=ros::Time::now();
                ros_pub->output_format==2 ? msg.header.frame_id=std::string("local") : msg.header.frame_id=std::string("world");
                sscanf ((char*)buff,"%lu\t%lf\t%lf\t%lf\t%lf\t%d\t%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf",&msg.week, &msg.tow, &msg.data[0], &msg.data[1], &msg.data[2], &msg.Q, &msg.ns, &msg.std[0],	&msg.std[1],
                        &msg.std[2],&msg.std[3],&msg.std[4],&msg.std[5], &msg.age, &msg.ratio);
                ros_pub->publisher.publish(msg);
            }
            ...
            while (ptr<=(char*)buff+n-2){
                ...
                if(single_msg[3]=='R' && single_msg[4]=='M' && single_msg[5]=='C'){ //GPRMC
                  ...
                  if(*ptr1!=','){
                      ...
                      rtkrcv_ros::gsv_sat gsv_sat;
                      rtkrcv_ros::nmea_gpgsv *msg_ptr;
                      if(single_msg[2]=='P'){
                          msg_ptr=&msg.gpgsv;
                      }else if(single_msg[2]=='L'){
                          msg_ptr=&msg.glgsv;
            ...
            ros_pub->publisher.publish(msg);
              ...
              if(ros_pub->time_format==1){
                  ...
                  rtkrcv_ros::states_hms msg;
                  msg.header.stamp=ros::Time::now();
                  ros_pub->output_format==2 ? msg.header.frame_id=std::string("local") : msg.header.frame_id=std::string("VEL_ENU");
                  sscanf ((char*)buff,"%lu/%lu/%lu\t%lu:%lu:%lf\t%lf\t%lf\t%lf\t%d\t%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf",&msg.year, &msg.month, &msg.day, &msg.hour,
                              &msg.min, &msg.sec, &msg.data[0], &msg.data[1], &msg.data[2], &msg.Q, &msg.ns, &msg.std[0],	&msg.std[1],
                              &msg.std[2],&msg.std[3],&msg.std[4],&msg.std[5], &msg.age, &msg.ratio);
                  ros_pub->publisher.publish(msg);
