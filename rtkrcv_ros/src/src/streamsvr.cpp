...
#include <ros/ros.h>
#include "rtklib/CorrectionsRTCM.h"

static void *strsvrthreadRos(void *arg);
int strsvrstartRos(strsvr_t *svr, int *opts, int *strs, char **paths,
...
static void *strsvrthreadRos(void *arg)
{
    ...
    rtcm_pub=node.advertise<rtklib::CorrectionsRTCM>("corr_rtcm", 1000);
    while (svr->state) {
        tick=tickget();
        ...
        if(n>0)
        {
            rtklib::CorrectionsRTCM msg;
            msg.header.stamp=ros::Time::now();
            for(int i=0;i<n;i++)     msg.data.push_back(svr->buff[i]);
...
/*-----------------------------------------------------------------------------*/
extern int strsvrstartRos(strsvr_t *svr, int *opts, int *strs, char **paths,
                       strconv_t **conv, const char *cmd, const double *nmeapos)
{
    int i,rw,stropt[5]={0};
    char file1[MAXSTRPATH],file2[MAXSTRPATH],*p;
...
#ifdef WIN32
    if (!(svr->thread=CreateThread(NULL,0,strsvrthreadRos(,svr,0,NULL))) {
#else
    if (pthread_create(&svr->thread,NULL,strsvrthreadRos,svr)) {
#endif
        for (i=0;i<svr->nstr;i++) strclose(svr->stream+i);
        return 0;
    }
