...
/* output solution as the form of ve/vn/vu------------------------------*/
static int out_enu_vel (unsigned char *buff, const char *s, const sol_t *sol,
                  const double *rb, const solopt_t *opt)
{
    double pos[3],vel_rover[3], vel_base[3],P[9],Q[9];
    int i;
    const char *sep=opt2sep(opt);
    char *p=(char *)buff;
    ...
    P[0]     =sol->covvel_ecef[0]; /* Ve */
    P[4]     =sol->covvel_ecef[1]; /* Vn */
    P[8]     =sol->covvel_ecef[2]; /* Vu */
    P[1]=P[3]=sol->covvel_ecef[3]; /* Ven */
    P[5]=P[7]=sol->covvel_ecef[4]; /* Vnu */
    P[2]=P[6]=sol->covvel_ecef[5]; /* Vue */

    covenu(pos,P,Q);

    p+=sprintf(p,"%s%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n",
               s,sep,vel_rover[0],sep,vel_rover[1],sep,vel_rover[2],sep,sol->stat,sep,sol->ns,sep,
               SQRT(Q[0]),sep,SQRT(Q[4]),sep,SQRT(Q[8]),sep,sqvar(Q[1]),
               sep,sqvar(Q[5]),sep,sqvar(Q[2]),sep,sol->age,sep,sol->ratio);
    return p-(char *)buff;
}
...
extern int outsolheads(unsigned char *buff, const solopt_t *opt)
{
    ...
    }else if (opt->posf==SOLF_ENU_VEL) { /* e/n/u-baseline */
      p+=sprintf(p,"%14s%s%14s%s%14s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%6s%s%6s\n",
                 "V_East(m/s)",sep,"V North(m/s)",sep,"V Up(m/s)",sep,
                 "Q",sep,"ns",sep,"std_VE(m/s)",sep,"std_VN(m/s)",sep,"std_VU(m/s)",sep,
                 "std_VEN(m/s)",sep,"std_VNU(m/s)",sep,"std_VEU(m/s)",sep,"age(s)",sep,"ratio");
    }
    return p-(char *)buff;
}
...
extern int outsols(unsigned char *buff, const sol_t *sol, const double *rb,
                   const solopt_t *opt, const ssat_t *ssat)
{
    gtime_t time,ts={0};
    double gpst;
    int week,timeu;
    ...
    switch (opt->posf) {
      ...
      break;
        case SOLF_ENU_VEL: p+=out_enu_vel(p,s,sol,rb,opt); break;
    }
    return p-buff;
}
                   ...
                   const solopt_t *opt)
{
    ...
    if ((n=outsols(buff,sol,rb,opt, NULL))>0) {
        fwrite(buff,n,1,fp);
    }
}
/* output solution extended ----------------------------------------------------
