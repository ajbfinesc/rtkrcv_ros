...
#include <ros/ros.h>
#include <string>
/*#include "rtklib/vel_enu_hms.h"
#include "rtklib/vel_enu_tow.h"
#include "rtklib/enu_tow.h"
#include "rtklib/enu_hms.h"
#include "rtklib/lla_tow.h"
#include "rtklib/lla_hms.h"
#include "rtklib/xyz_tow.h"
#include "rtklib/xyz_hms.h"*/
#include "rtklib/pos_tow.h"
#include "rtklib/pos_hms.h"
/*#include "rtklib/nmea_gpgga.h"
#include "rtklib/nmea_gpgsa.h"
#include "rtklib/nmea_gprmc.h"
#include "rtklib/nmea_gpgsv.h"*/
#include "rtklib/nmea.h"
...
/* open serial ---------------------------------------------------------------*/
static serial_t *openserial(char *path, int mode, char *msg)
{
    const int br[]={
    ...
    printf("c_cflag=%x /t bs[i]=%d\t rw=%d\n", ios.c_cflag, i, rw);
...
/* open ros publisher -----------------------------------------------------------*/
static ros_pub_t *openRosPub(const char *path, int out_format, int time_format, char *msg)
{
    ros_pub_t *ros_pub=new ros_pub_t;
    ros_pub->output_format=out_format;

    if(out_format==0 || out_format==1 || out_format==2){
        ros_pub->time_format=time_format;
        if(time_format==1){
            ros_pub->publisher=node->advertise<rtklib::pos_hms>(path,10);
            return ros_pub;
        }else{
            ros_pub->publisher=node->advertise<rtklib::pos_tow>(path,10);
            return ros_pub;
        }
    }else if(out_format==3){
        ros_pub->publisher=node->advertise<rtklib::nmea>(path,10);
        return ros_pub;
    }else if(out_format==5){
        ros_pub->time_format=time_format;
        if(time_format==1){
            ros_pub->publisher=node->advertise<rtklib::pos_hms>(path,10);
            return ros_pub;
...
/* publish ros message ----------------------------------------------------------*/
static int publish_ros_msg(ros_pub_t *ros_pub, unsigned char *buff, int n)
{

    printf("publish_ros_msg stream.c\tROS Output format=%d\n\n", ros_pub->output_format);
    if(n>0){

        if(ros_pub->output_format==0 || ros_pub->output_format==1 || ros_pub->output_format==2){

            if(ros_pub->time_format==1){
                rtklib::pos_hms msg;
                msg.header.stamp=ros::Time::now();
                ros_pub->output_format==2 ? msg.header.frame_id=std::string("local") : msg.header.frame_id=std::string("world");
                sscanf ((char*)buff,"%lu/%lu/%lu\t%lu:%lu:%lf\t%lf\t%lf\t%lf\t%d\t%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf",&msg.year, &msg.month, &msg.day, &msg.hour,
                            &msg.min, &msg.sec, &msg.pos.x, &msg.pos.y, &msg.pos.z, &msg.Q, &msg.ns, &msg.std_N,	&msg.std_E, &msg.std_U, &msg.std_NE, &msg.std_EU,
                            &msg.std_UN, &msg.age, &msg.ratio);
                ros_pub->publisher.publish(msg);
            }else{
                rtklib::pos_tow msg;
                msg.header.stamp=ros::Time::now();
                ros_pub->output_format==2 ? msg.header.frame_id=std::string("local") : msg.header.frame_id=std::string("world");
                sscanf ((char*)buff,"%lu\t%lf\t%lf\t%lf\t%lf\t%d\t%d\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf",&msg.week, &msg.tow, &msg.pos.x, &msg.pos.y,
                              &msg.pos.z, &msg.Q, &msg.ns, &msg.std_N,	&msg.std_E, &msg.std_U, &msg.std_NE, &msg.std_EU, &msg.std_UN, &msg.age, &msg.ratio);
                ros_pub->publisher.publish(msg);
            }
        }else if(ros_pub->output_format==3){
            /*NMEA*/
            NMEA_GPRMC rmc;
            NMEA_GPGGA gga;
            rtklib::nmea msg;
            ...
            char single_msg[1024];
            char *ptr=(char*)buff;
            char *ptr_prev=ptr;
            while (ptr<=(char*)buff+n-2){
                bzero(single_msg,1024);
                while( *(ptr)!='\r')
                    ptr++;

                strncpy(single_msg, ptr_prev, ptr-ptr_prev);
                ptr+=2;

                if(single_msg[3]=='R' && single_msg[4]=='M' && single_msg[5]=='C'){ //GPRMC
                    sscanf((char*)buff,"$GPRMC,%f,%c,%lf,%c,%lf,%c,%f,%f,%d,%f,%c,%c*%s\n",
                            &rmc.time,&rmc.validity,&rmc.latitude,&rmc.char_lat,&rmc.longitude,&rmc.char_lon,&rmc.speed,&rmc.course,&rmc.date,&rmc.variation,&rmc.east_west,&rmc.mode,&rmc.checksum);
                    msg.gprmc.hour=(int)rmc.time/10000;
                    msg.gprmc.min=(int)rmc.time/100-msg.gprmc.hour*100;
                    msg.gprmc.sec=rmc.time-msg.gprmc.min*100-msg.gprmc.hour*10000;
                    rmc.validity=='A' ? msg.gprmc.validity=msg.gprmc.A : msg.gprmc.validity=msg.gprmc.V;
                    msg.gprmc.lat.deg=(int)rmc.latitude/100;
                    msg.gprmc.lat.min=(int)rmc.latitude-msg.gprmc.lat.deg*100;
                    msg.gprmc.lat.sec=(rmc.latitude-msg.gprmc.lat.deg*100-msg.gprmc.lat.min)*60.0;
                    rmc.char_lat=='N' ? msg.gprmc.lat.direction=msg.gprmc.lat.NORTH : msg.gprmc.lat.direction=msg.gprmc.lat.SOUTH;
                    msg.gprmc.lat.coord_deg=(msg.gprmc.lat.deg+msg.gprmc.lat.min/60.0+msg.gprmc.lat.sec/60.0/60.0);
                    msg.gprmc.lat.coord_radians= msg.gprmc.lat.coord_deg*M_PI/180;

                    msg.gprmc.lon.deg=(int)rmc.longitude/100;
                    msg.gprmc.lon.min=(int)rmc.longitude-msg.gprmc.lon.deg*100;
                    msg.gprmc.lon.sec=(rmc.longitude-msg.gprmc.lon.deg*100-msg.gprmc.lon.min)*60.0;
                    rmc.char_lon=='W' ? msg.gprmc.lon.direction=msg.gprmc.lon.WEST : msg.gprmc.lon.direction=msg.gprmc.lon.EAST;
                    msg.gprmc.lon.coord_deg=(msg.gprmc.lon.deg+msg.gprmc.lon.min/60.0+msg.gprmc.lon.sec/60.0/60.0);
                    msg.gprmc.lon.coord_radians= msg.gprmc.lon.coord_deg*M_PI/180;

                    msg.gprmc.day=(int)rmc.date/10000;
                    msg.gprmc.month=(int)rmc.date/100-msg.gprmc.day*100;
                    msg.gprmc.year=rmc.date-msg.gprmc.month*100-msg.gprmc.day*10000;
                    msg.gprmc.speed=rmc.speed;
                    msg.gprmc.course=rmc.course;
                    msg.gprmc.mag_variation=rmc.variation;
                    msg.gprmc.mag_variation_dir=rmc.east_west;
                    msg.gprmc.mode=rmc.mode;
                    msg.gprmc.checksum=std::string((char*)rmc.checksum);

                    msg.gprmc.string_msg=std::string(single_msg);


                } else if(single_msg[3]=='G' && single_msg[4]=='G' && single_msg[5]=='A'){ //GPGGA
                    sscanf((char*)single_msg,"$GPGGA,%f,%lf,%c,%lf,%c,%d,%d,%lf,%lf,%c,%lf,%c,%lf,%s*%s\n",
                            &gga.time,&gga.latitude,&gga.char_lat,&gga.longitude,&gga.char_lon,&gga.quality,&gga.nsat,&gga.hdop,&gga.altitude,&gga.alt_units,
                            &gga.undulation,&gga.u_units,&gga.age, &gga.station_id, &gga.checksum);
                    msg.gpgga.hour=(int)gga.time/10000;
                    msg.gpgga.min=(int)gga.time/100-msg.gpgga.hour*100;
                    msg.gpgga.sec=gga.time-msg.gpgga.min*100-msg.gpgga.hour*10000;
                    msg.gpgga.lat.deg=(int)gga.latitude/100;
                    msg.gpgga.lat.min=(int)gga.latitude-msg.gpgga.lat.deg*100;
                    msg.gpgga.lat.sec=(gga.latitude-msg.gpgga.lat.deg*100-msg.gpgga.lat.min)*60.0;
                    gga.char_lat=='N' ? msg.gpgga.lat.direction=msg.gpgga.lat.NORTH : msg.gpgga.lat.direction=msg.gpgga.lat.SOUTH;

                    msg.gpgga.lat.coord_deg=(msg.gpgga.lat.deg+msg.gpgga.lat.min/60.0+msg.gpgga.lat.sec/60.0/60.0);
                    msg.gpgga.lat.coord_radians= msg.gpgga.lat.coord_deg*M_PI/180;

                    msg.gpgga.lon.deg=(int)gga.longitude/100;
                    msg.gpgga.lon.min=(int)gga.longitude-msg.gpgga.lon.deg*100;
                    msg.gpgga.lon.sec=(gga.longitude-msg.gpgga.lon.deg*100-msg.gpgga.lon.min)*60.0;
                    gga.char_lon=='W' ? msg.gpgga.lon.direction=msg.gpgga.lon.WEST : msg.gpgga.lon.direction=msg.gpgga.lon.EAST;
                    msg.gpgga.lon.coord_deg=(msg.gpgga.lon.deg+msg.gpgga.lon.min/60.0+msg.gpgga.lon.sec/60.0/60.0);
                    msg.gpgga.lon.coord_radians= msg.gpgga.lon.coord_deg*M_PI/180;

                    msg.gpgga.Q=gga.quality;
                    msg.gpgga.ns=gga.nsat;
                    msg.gpgga.HDOP=gga.hdop;
                    msg.gpgga.height_msl=gga.altitude;
                    msg.gpgga.unit=gga.alt_units;
                    msg.gpgga.geod_height=gga.undulation;
                    msg.gpgga.age=gga.age;

                    if(gga.station_id[0]=='*'){
                        msg.gpgga.base_id=std::string("");
                        gga.checksum[0]=gga.station_id[1];
                        gga.checksum[1]=gga.station_id[2];
                        msg.gpgga.checksum=std::string((char*)gga.checksum);
                    }else{
                        msg.gpgga.base_id=std::string(gga.station_id);
                        msg.gpgga.checksum=std::string((char*)gga.checksum);
                    }
                    msg.gpgga.string_msg=std::string(single_msg);

              }else if(single_msg[3]=='G' && single_msg[4]=='S' && single_msg[5]=='A'){ //G*GSA


                  rtklib::nmea_gpgsa *msg_ptr;
                  if(single_msg[2]=='P'){
                      msg_ptr=&msg.gpgsa;
                  }else if(single_msg[2]=='L'){
                      msg_ptr=&msg.glgsa;
                  }else if(single_msg[2]=='A'){
                      msg_ptr=&msg.gagsa;
                  }
                  ...
                  char *ptr1=single_msg+7;

                  if(*ptr1!=','){
                      char1_aux=*ptr1;
                      msg_ptr->mode_1.push_back(char1_aux);
                      ptr1+=2;
                      if(*ptr1!=','){
                          char1_aux=*ptr1;
                          msg_ptr->mode_2.push_back(char1_aux);
                          ptr1+=2;
                          for (int i=0;i<12;i++){
                              if(*ptr1!=','){
                                  strncpy(char2_aux,ptr1,2);
                                  msg_ptr->PRN.push_back(atoi((char*)&char2_aux));
                                  ptr1+=3;
                              }else{
                                ptr1++;
                              }
                          ...
                          if(*ptr1!=','){
                              strncpy(char3_aux,ptr1,3);
                              msg_ptr->PDOP=atof(char3_aux);
                              while(*ptr1!=',') ptr1++;
                          }
                          ptr1++;
                          if(*ptr1!=','){
                              strncpy(char3_aux,ptr1,3);
                              msg_ptr->HDOP=atof(char3_aux);
                              while(*ptr1!=',') ptr1++;
                          }
                          ptr1++;
                          if(*ptr1!=','){
                              strncpy(char3_aux,ptr1,3);
                              msg_ptr->VDOP=atof(char3_aux);
                              while(*ptr1!='*') ptr1++;
                          }
                          ptr1++;
                          msg_ptr->checksum.push_back(*ptr1);
                          msg_ptr->checksum.push_back(*(ptr1+1));
                          }
                      }
                      msg_ptr->string_msg=std::string(single_msg);

                }else if(single_msg[3]=='G' && single_msg[4]=='S' && single_msg[5]=='V'){ //G*GSV


                      rtklib::gsv_sat gsv_sat;
                      rtklib::nmea_gpgsv *msg_ptr;
                      if(single_msg[2]=='P'){
                          msg_ptr=&msg.gpgsv;
                      }else if(single_msg[2]=='L'){
                          msg_ptr=&msg.glgsv;
                      ...
                      if(*ptr1!=','){
                          ...
                          for (int j=0;j<2;j++){ // discard #messages and message_sequence fields
                              while (*ptr2!=',') ptr2++;
                              ptr1=++ptr2;
                          }
                          while (*ptr2!=',') ptr2++;
                          bzero(char3_aux, 4);
                          strncpy(char3_aux,ptr1,ptr2-ptr1);
                          msg_ptr->n_sat=atoi((char*)char3_aux);
                          ...
                          while(*ptr1!='*' && *(ptr1+1)!='*'){
                              ...
                              if(ptr2-ptr1>1){
                                  bzero(char3_aux, 4);
                                  strncpy(char3_aux,ptr1,ptr2-ptr1);
                                  gsv_sat.PRN=atoi((char*)char3_aux);

                                  ptr1=++ptr2;
                                  while(*ptr2!=',') ptr2++;
                                  bzero(char3_aux, 4);
                                  strncpy(char3_aux,ptr1,ptr2-ptr1);
                                  gsv_sat.elevation=atoi((char*)char3_aux);
                                  ptr1=++ptr2;

                                  while(*ptr2!=',') ptr2++;
                                  bzero(char3_aux, 4);
                                  strncpy(char3_aux,ptr1,ptr2-ptr1);
                                  gsv_sat.azimuth=atoi((char*)char3_aux);

                                  ptr1=++ptr2;
                                  while(*ptr2!=',') ptr2++;
                                  bzero(char3_aux, 4);
                                  strncpy(char3_aux,ptr1,ptr2-ptr1);
                                  gsv_sat.snr=atoi((char*)char3_aux);
                                  msg_ptr->sats.push_back(gsv_sat);
                                  ptr1=++ptr2;
                              }else{
                                  ptr2=ptr2+4;
                                  ptr1=ptr2;
            ...
            ros_pub->publisher.publish(msg);
        }else if(ros_pub->output_format==5){
            //VEL
          printf("VEL MODE\n\n");
//          ros_pub->publisher.publish(msg);
        }
...
static int writetcpsvr(tcpsvr_t *tcpsvr, unsigned char *buff, int n, char *msg)
{
    ...
    for(i=0;i<n;i++)
		msg_to_parse[i]=*(buff+i);
		
    /*if(n>0)
	{
		ptr=buff;
		rtklib::rtkrcv_solution msg_ros;
		msg_ros.header.stamp=ros::Time::now();
		//for( ;ptr<buff+n;ptr++){     
			//printf("%c ",(char)*ptr);
			//msg.data.push_back(*ptr);
			int ret=sscanf(msg_to_parse,"%hu/%hu/%hu %hu:%hu:%lf %lf %lf %f %d %d %f %f %f %f %f %f %f %f",&msg_ros.year, &msg_ros.month, &msg_ros.day, &msg_ros.hour, 
			&msg_ros.min, &msg_ros.dec_sec, &msg_ros.x, &msg_ros.y, &msg_ros.z, &msg_ros.fix_status,
			&msg_ros.n_sats, &msg_ros.x_std, &msg_ros.y_std, &msg_ros.z_std, &msg_ros.xy_std, &msg_ros.yz_std, &msg_ros.xz_std,
			&msg_ros.age_diff, &msg_ros.ar_ratio);
...
*-----------------------------------------------------------------------------*/
extern int stropen(stream_t *stream, int type, int mode, char *path)
{
...
/*        case STR_ROS_MSG  :
            if(mode==STR_MODE_W){
              ...
              stream->port=openRosPub(path,  stream->msg);
            }
        break;*/
        default: 
        stream->state=0; return 1;
    }
...
extern int stropen_out_sol(stream_t *stream, int type, int mode, char *path, int out_format, int time_format)
{
    ...
    switch (type) {
        ...
        case STR_ROS_MSG  :
            if(mode==STR_MODE_W){
              /*Write mode: create a ros publisher*/

              printf("stropen_out_sol: type=%d mode=%d path=%s format=%d time_format=%d\n",type,mode,path, out_format, time_format);
              stream->port=openRosPub(path, out_format, time_format,  stream->msg);
            }
        break;
        default:
        stream->state=0; return 1;
...
extern int strwrite(stream_t *stream, unsigned char *buff, int n)
{
    ...
    switch (stream->type) {
        ...
        case STR_ROS_MSG  :
          /*Write mode: create a ros publisher*/
          printf("strwrite stream.cpp \t buffer=\n%s\n\n", buff);
          ns=publish_ros_msg((ros_pub_t*) stream->port,buff,n);				break;
        case STR_FTP     :
        case STR_HTTP    :
        default:
            strunlock(stream);
