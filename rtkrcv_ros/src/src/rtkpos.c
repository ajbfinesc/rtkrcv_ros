                  ...
                  const nav_t *nav)
{
    ...
    if (stat==SOLQ_FIX) {
        for (i=0;i<3;i++) {
            ...
            if (rtk->na>=6){
                rtk->sol.vel_ecef[i]=rtk->xa[i+3];
                rtk->sol.covvel_ecef[i]=(float)rtk->Pa[i+3+(i+3)*rtk->na];
            }
        }
        ...
        if (rtk->na>=6){
            rtk->sol.covvel_ecef[3]=(float)rtk->Pa[4+3*rtk->na];
            rtk->sol.covvel_ecef[4]=(float)rtk->Pa[4+5*rtk->na];
            rtk->sol.covvel_ecef[5]=(float)rtk->Pa[5+3*rtk->na];
        }else{
            rtk->sol.vel_ecef[0]=rtk->sol.vel_ecef[1]=rtk->sol.vel_ecef[2]=0;
            for (i=0;i<6;i++){
                rtk->sol.covvel_ecef[i]=0;
            }
        }
    }
    else {
        for (i=0;i<3;i++) {
            ...
            if (rtk->nx>=6){
                rtk->sol.vel_ecef[i]=rtk->x[i+3];
                rtk->sol.covvel_ecef[i]=(float)rtk->P[i+3+(i+3)*rtk->nx];
            }
        }
        ...
        if (rtk->nx>=6){
            rtk->sol.covvel_ecef[3]=(float)rtk->P[4+3*rtk->nx];
            rtk->sol.covvel_ecef[4]=(float)rtk->P[4+5*rtk->nx];
            rtk->sol.covvel_ecef[5]=(float)rtk->P[5+3*rtk->nx];
        }else{
            rtk->sol.vel_ecef[0]=rtk->sol.vel_ecef[1]=rtk->sol.vel_ecef[2]=0;
            for (i=0;i<6;i++){
                rtk->sol.vel_ecef[i]=0;
            }
        }
