...
static void writesol(rtksvr_t *svr, int index)
{
    ...
    for (i=0;i<2;i++) {
        ...
        /* output solution */
        n=outsols(buff,&svr->rtk.sol,svr->rtk.rb,svr->solopt+i, svr->rtk.ssat);
...
#endif
{
    ...
    if ( ! svr->rtk.opt.syncsol){ /*Original version, solutions are not synchronized*/
                       ...
                       solopt_t *solopt, stream_t *moni)
{
    ...
    for (i=0;i<8;i++) {
        ...
        if(i==3 || i==4){ /*Open output solution streams*/

          if(!svr->rtk.opt.dynamics && svr->solopt[i-3].posf==5){
            /* Velocity output stream selected but rover dynamics mode is not enabled */
            printf("\n\nERROR - a velocity output stream can only be selected if rover dynamics mode is enabled.\nPlease change variable pos1-dynamics in the configuration file\n\n\n");
            for (i--;i>=0;i--) strclose(svr->stream+i);
            return 0;
          }
          if (!stropen_out_sol(svr->stream+i,strs[i],rw,paths[i], (svr->solopt+i-3)->posf, (svr->solopt+i-3)->timef)) {
                for (i--;i>=0;i--) strclose(svr->stream+i);
                return 0;
            }
          }else{
        ...
        /* set initial time for input rtcm and raw streams */
        if (i<3) {
