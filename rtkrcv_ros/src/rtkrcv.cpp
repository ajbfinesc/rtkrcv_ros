...
static int soltype      =0;             /* sol format (0:dms,1:deg,2:xyz,3:enu,4:pyl,5:vel_enu) */
...
#define OSTOPT  "0:off,1:serial,2:file,3:tcpsvr,4:tcpcli,6:ntripsvr,10:ros"
...
#define SOLOPT  "0:llh,1:xyz,2:enu,3:nmea,4:gisf,5:vel_enu"
...
static void prsolution(vt_t *vt, const sol_t *sol, const double *rb)
{
    ...
    if (soltype==0) {
        ...
        double vel_rover[3], vel_base[3];
        ecef2pos(rb,pos);
        ecef2enu(pos,sol->vel_ecef,vel_rover);
        ecef2enu(pos,rb+3,vel_base);

        for (i=0;i<3;i++) vel_rover[i]-=vel_base[i];

        vt_printf(vt," VE:%8.5f",vel_rover[0]);
        vt_printf(vt," VN:%8.5f",vel_rover[1]);
        vt_printf(vt," VU:%8.5f",vel_rover[2]);
    }
    if (solflag&2) {
...
static void prstream(vt_t *vt)
{
    ...
    const char *type[]={
        "-","serial","file","tcpsvr","tcpcli","udp","ntrips","ntripc","ftp","http","ros"
    };
    const char *fmt[]={"rtcm2","rtcm3","oem4","oem3","ubx","ss2","hemis","skytreq",
                       "gw10","javad","nvs","binex","rt17","","","sp3","","",""};
    const char *sol[]={"llh","xyz","enu","nmea","gisf","vel"};
    stream_t stream[9];
    int i,format[9]={0};
...
{
    trace(3,"cmd_start:\n");
    vt_printf(vt,"Start command received by ROS service\n");
    if (!startsvr(vt)){
      *res="rtk server start error";
      return false;
    }
    *res="rtk server start";
    vt_printf(vt,"rtk server start\n");
    return true;
}
/* stop command for the ROS service --------------------------------------------------------------*/
bool srv_cmd_stop(char **args, int narg, vt_t *vt, std::string *res)
{
    vt_printf(vt,"Stop command received by ROS service\n");
    trace(3,"cmd_stop:\n");
...
/* restart command for the ROS service -----------------------------------------------------------*/
bool srv_cmd_restart(char **args, int narg, vt_t *vt, std::string *res)
{
    trace(3,"cmd_restart:\n");
    vt_printf(vt,"Restart command received by ROS service\n");
    stopsvr(vt);
    if (!startsvr(vt)) {
      *res="rtk server restart error";
      return false;
    }
    *res="rtk server restart";
    vt_printf(vt,"rtk server restart\n");
    return true;
}
...
/* load command for the ROS service--------------------------------------------------------------*/
bool srv_cmd_load(char **args, int narg, vt_t *vt, std::string *res)
{
    char file[MAXSTR]="";
    ...
    vt_printf(vt,"options loaded from %s. restart to enable them\n",file);
    *res="Options loaded. Restart to enable them";
    return true;
}
